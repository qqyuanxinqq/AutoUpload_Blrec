...
OAUTH2_URL = "https://passport.bilibili.com/api/v3/oauth2/login"
...
def login_by_access_token(access_token):
    ...
    #     'access_token': access_token,
    #     'appkey': APPKEY,
    #     'refresh_token' : '3f72b18127b7bf56b2de7af93df6f922'
    #     }
    # post_data['sign'] = cipher.sign_dict(post_data, APPSECRET)
    # r = requests.post(
    #     url='https://passport.bilibili.com/api/v2/oauth2/refresh_token',
    ...
    # print(b'\xe8\xaf\xb7\xe6\xb1\x82\xe9\x94\x99\xe8\xaf\xaf'.decode('utf-8'))
    # raise Exception()

    # return 'hqvb6wfa', login_data['mid'], login_data["expires_in"]
...
def metered_upload_chunk(upload_url, server_file_name, local_file_name, chunk_data, chunk_size, chunk_id, chunk_total_num, upload_rate = 0, max_retry = 5):
    """
    upload video chunk.
    Args:
...
def metered_upload_video_part(access_token, sid, mid, video_part: VideoPart, upload_rate = 0, max_retry=5, thread_pool_workers = 1) -> Optional[str]:
    """
    upload a video file.
    Args:
        ...
        upload_rate: upload rate in B/s, 0 means no limit
...
def parse_video_title(form:Optional[str], item) -> str:
    ...
    if form and item.get('start_time') and item.get('live_title'):
        info_dict = dict()
        ...
        return os.path.basename(item["filename"]).split('.')[0]
...
class BilibiliUploaderBase():
    ...
    upload_rate: upload rate in kB/s, 0 means no limit
    """
    ...
    def _upload(self):
        ...
        if dynamic_update:
            live_info = Live(filename=self.video_list_json)
        ...
        while True:
            ...
            #     print("Trakcing provided record_info json file.")
                ...
                for item in live_info._data["video_list"][post_videos_num::]:   #??? Warning, operations on Live._data
            ...
            if dead_loop_count > 3*60*60/40:
                print("No new videos for 180 minutes, stop waiting for new videos.")
                break
